...
def read_grid(grid):
  """Parse the grid variable into a numpy matrix"""
  matrix = numpy.array([])
  for line in grid.split('\n'):
    if not line=='':
      if not matrix.shape==(0,):  # Check if it is not the first line
        matrix = numpy.vstack([matrix,numpy.fromstring(line,sep=' ')])
      else:
        matrix = numpy.fromstring(line,sep=' ')
  return matrix

def main():
  """
  Calculate the largest product in the grid of four consecutive numbers, this
